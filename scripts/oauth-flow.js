 ...
 * Manages the complete OAuth 2.0 flow with Notion
 */

require('dotenv').config();

const { NotionOAuthHandler } = require('../services/notion-setup/oauth-handler.ts');
...
function showAuthorizationUrl(handler) {
  ...
  console.log('   http://localhost:3000/oauth-callback?code=YOUR_CODE&state=XXX\n');
  console.log('5. Copy the "code" value');
  console.log('6. Run: npm run oauth-exchange -- YOUR_CODE\n');
}

async function exchangeCode(handler, code) {
  try {
...
async function refreshToken(handler, refreshTokenValue) {
  try {
...
function listSavedWorkspaces(handler) {
  const workspaces = handler.getSavedWorkspaces();
...
function removeWorkspace(handler, workspaceId) {
